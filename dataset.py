...
class RedditDataset:
    ...
    def __init__(self, raw_dir, processed_dir, self_loop=True, reverse_edge=True,interval=1):
        self.raw_dir = raw_dir
        self.processd_dir = processed_dir
        self.reverse_edge = reverse_edge
    ...
    def process_raw_data(self):
        ...
        with open(PATH) as f:
            ...
            for idx, line in enumerate(f):
                ...
                feat = numpy.array([float(x) for x in e[4:]])  # features
        ...
        id_features=pandas.DataFrame({'idx':idx_list,'features': feat_l})
        ...
        zero_shape = id_features.loc[:,'features'][0].shape[0]
        for i in range(upper_u + 1, srt_dst_time.i.max() + 1):
            id_label = id_label.append({'idx': i, 'label': 2}, ignore_index=True)
            id_features = id_features.append({ 'idx' : i ,'features' : numpy.zeros(zero_shape)})
        ...
        if interval > 1 :
            ...
            srt_dst_time.ts = int((max_time_stamp- min_time_stamp) / time_slice)
        ...
        numpy.save(src_dst_time_path, src_dst_time)
        print("Process Reddit raw data done, data has saved into {}".format(processed_dir))
        # 将转换好的数据储存起来
